...
if($action == 'edit' || $preview) {
	...
	$CON_FORM_END = '</form>';
	$CON_TEXTAREA = '<textarea class="contentTextarea" name="content" style="width:100%" cols="100" rows="30" autofocus onfocus="this.value = this.value;">'.h(str_replace("&lt;", "<", $CON)).'</textarea>';
	$CON_PREVIEW = '<input class="submit" type="submit" name="preview" accesskey="q" value="'.$T_PREVIEW.'"/>';
...
$tpl_subs = array(
	...
	'SEARCH_SUBMIT' => !$action ? ("<input class=\"submit\" type=\"submit\" value=\"$T_SEARCH\" autofocus />") : ("<input class=\"submit\" type=\"submit\" value=\"$T_SEARCH\"/>"),
